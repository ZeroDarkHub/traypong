...
// ── Exact constants from physics.js ──────────────────────────────────────────
const CW = 300, CH = 380;
const PW = 56, PH = 8, BR = 5;
const PY_PLAYER = CH - 24;
const PY_AI = 16;

// ── Drawing helpers — exact copies from GameCanvas.jsx ───────────────────────
...
function stepDemo(state, dt) {
  ...
  if (vy > 0 && y + BR >= PY_PLAYER && y + BR <= PY_PLAYER + PH && x > playerX && x < playerX + PW) {
    vy = -Math.abs(vy) * 1.04;
    y = PY_PLAYER - BR;
  }
  if (vy < 0 && y - BR <= PY_AI + PH && y - BR >= PY_AI && x > aiX && x < aiX + PW) {
    vy = Math.abs(vy) * 1.04;
    y = PY_AI + PH + BR;
  }
  if (y > CH + 20 || y < -20) {
  ...
  const aiDiff = (x - PW / 2) - aiX;
  aiX = Math.max(0, Math.min(CW - PW, aiX + Math.sign(aiDiff) * Math.min(Math.abs(aiDiff) * 0.09, 3.2 * dt)));
  const plDiff = (x - PW / 2) - playerX;
  playerX = Math.max(0, Math.min(CW - PW, playerX + Math.sign(plDiff) * Math.min(Math.abs(plDiff) * 0.07, 3.8 * dt)));
  return { ...state, ball: { x, y, vx, vy, trail }, playerX, aiX, waypointIdx };
}

// ── App mockup with live canvas ───────────────────────────────────────────────
function AppMockup() {
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        borderRadius: "12px 12px 0 0", padding: "8px 16px",
      ...
      <div style={{
        ...
        {/* Score bar */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, padding: "10px 20px 8px", background: "#0d0d0f" }}>
        ...
        {/* Live canvas */}
        <canvas ref={canvasRef} width={CW} height={CH} style={{ display: "block", margin: "0 20px 20px" }} />
      </div>
    </div>
  );
}
...
function FeatureCard({ icon, title, desc, tag }) {
  ...
  return (
    ...
    >
      {hovered && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(to right, transparent, rgba(153,102,255,0.4), transparent)" }} />}
      ...
      <span style={{ display: "inline-block", marginTop: 14, fontSize: 10, letterSpacing: "0.08em", padding: "3px 8px", borderRadius: 3, background: "rgba(153,102,255,0.1)", color: "#c299ff", border: "1px solid rgba(153,102,255,0.15)" }}>{tag}</span>
    </div>
  );
}
    ...
    <div style={{ background: "#080810", color: "#f0f0fa", fontFamily: "'DM Mono', monospace", WebkitFontSmoothing: "antialiased", position: "relative", overflowX: "hidden" }}>
      ...
      <section style={{ minHeight: "92vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "clamp(60px, 8vh, 80px) clamp(16px, 4vw, 24px) clamp(40px, 6vh, 60px)", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
          ...
          <div style={{ position: "absolute", top: "14%", left: "50%", width: "clamp(48px, 8vw, 72px)", height: "clamp(6px, 1.2vw, 8px)", borderRadius: 4, background: "#e84040", boxShadow: "0 0 20px rgba(232,64,64,0.4)", animation: "paddleAi 4s ease-in-out infinite" }} />
          <div style={{ position: "absolute", bottom: "14%", left: "50%", width: "clamp(48px, 8vw, 72px)", height: "clamp(6px, 1.2vw, 8px)", borderRadius: 4, background: "#f0f0fa", boxShadow: "0 0 20px rgba(240,240,255,0.3)", animation: "paddlePlayer 4s ease-in-out infinite 0.5s" }} />
          <div style={{ position: "absolute", width: "clamp(8px, 1.5vw, 10px)", height: "clamp(8px, 1.5vw, 10px)", borderRadius: "50%", background: "#fff", boxShadow: "0 0 16px rgba(200,170,255,0.9), 0 0 40px rgba(153,102,255,0.4)", left: "50%", top: "50%", animation: "ballFloat 4s linear infinite" }} />
          <div style={{ position: "absolute", width: "clamp(300px, 60vw, 600px)", height: "clamp(300px, 60vw, 600px)", borderRadius: "50%", background: "radial-gradient(circle, rgba(153,102,255,0.08) 0%, transparent 70%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)" }} />
        </div>

        <div style={{ position: "relative", zIndex: 2 }}>
          <p style={{ fontSize: "clamp(10px, 2.5vw, 11px)", letterSpacing: "0.18em", textTransform: "uppercase", color: "#9966ff", marginBottom: "clamp(16px, 3vw, 24px)", animation: "fadeUp 0.8s ease both" }}>
            macOS menu bar · instantly playable
          </p>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(42px, 9vw, 110px)", fontWeight: 800, lineHeight: 0.9, letterSpacing: "-0.02em", marginBottom: "clamp(20px, 4vw, 28px)", animation: "fadeUp 0.8s 0.1s ease both" }}>
      ...
      <section id="howto" style={{ padding: "clamp(60px, 8vh, 60px) clamp(24px, 5vw, 48px) clamp(80px, 10vh, 100px)", maxWidth: 900, margin: "0 auto" }}>
        ...
        <Reveal delay={0.05}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(32px, 6vw, 56px)", fontWeight: 800, lineHeight: 1, letterSpacing: "-0.02em", marginBottom: "clamp(40px, 6vw, 56px)" }}>
        ...
        <div style={{ paddingLeft: 56, position: "relative" }}>
          <div style={{ position: "absolute", left: 15, top: 20, bottom: 20, width: 1, background: "linear-gradient(to bottom, #9966ff, rgba(153,102,255,0.1))" }} />
          {STEPS.map(({ num, title, desc, warn }, i) => (
            <Reveal key={num} delay={i * 0.07}>
              <div style={{ position: "relative", padding: "28px 0", borderBottom: i < STEPS.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none" }}>
                <div style={{ position: "absolute", left: -56, top: 28, width: 32, height: 32, borderRadius: "50%", background: "#0f0f1a", border: "1px solid rgba(153,102,255,0.35)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#9966ff", fontWeight: 500 }}>{num}</div>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 10, color: "#f0f0fa", lineHeight: 1.3 }}>{title}</div>
                <p style={{ fontSize: 14, lineHeight: 1.75, color: "#888899" }}>{desc}</p>
                {warn && (
                  <div style={{ marginTop: 14, padding: "12px 16px", background: "rgba(232,160,40,0.06)", border: "1px solid rgba(232,160,40,0.18)", borderLeft: "3px solid rgba(232,160,40,0.5)", borderRadius: "0 6px 6px 0" }}>
                    <div style={{ fontSize: 11, fontWeight: 500, color: "#c8a050", marginBottom: 5, letterSpacing: "0.04em" }}>{warn.label}</div>
      ...
      {/* ── Download CTA ── */}
      <section id="download" style={{ padding: "clamp(80px, 12vh, 120px) clamp(24px, 5vw, 48px)", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: "clamp(400px, 80vw, 800px)", height: "clamp(200px, 40vw, 400px)", borderRadius: "50%", background: "radial-gradient(ellipse, rgba(153,102,255,0.1) 0%, transparent 70%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none" }} />
        <Reveal>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(36px, 8vw, 80px)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 0.95, marginBottom: "clamp(16px, 3vw, 24px)", position: "relative" }}>
            Ready to<br />play?
          </h2>
        </Reveal>
        <Reveal delay={0.1}>
          <p style={{ fontFamily: "'Instrument Serif', serif", fontStyle: "italic", fontSize: "clamp(16px, 4vw, 18px)", color: "#55556a", maxWidth: "clamp(280px, 80vw, 400px)", margin: "0 auto clamp(32px, 6vw, 48px)", position: "relative" }}>
            Your menu bar has been too boring for too long.
          </p>
        </Reveal>
        <Reveal delay={0.2}>
          <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            <a href="https://github.com/ZeroDarkHub/traypong/releases/download/v1.0.0/TrayPong-1.0.0-arm64.dmg" download style={{ display: "inline-flex", alignItems: "center", gap: 12, background: "#f0f0fa", color: "#080810", fontFamily: "'Syne', sans-serif", fontSize: 16, fontWeight: 700, letterSpacing: "0.02em", padding: "18px 40px", borderRadius: 8, textDecoration: "none", transition: "transform 0.15s, box-shadow 0.2s", boxShadow: "0 0 60px rgba(240,240,255,0.15)" }}
              onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 12px 60px rgba(240,240,255,0.25)"; }}
              onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 0 60px rgba(240,240,255,0.15)"; }}
            >
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M9 2v10M5 8l4 4 4-4M3 14h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
              Download for macOS
            </a>
